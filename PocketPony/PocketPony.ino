...
#define GOLODOVKA 2 
// how many hunger units are taken per hour (when using an external timer, based on a real-time clock)
...
int switchFRAZ;
bool nya;
byte MOOD; //4 - delight, 3 - smile, 2 - serious, 1 - sad 0 - calm
bool storm;
bool night;
byte select13menu=1;
bool up;
...
u8g.drawXBM( 0, 0, 8, 64, wallpaper);  // draw the background
...
//BODY (left)
u8g.setColorIndex(0); // paint over the body
...
u8g.drawLine(DERPx-2,DERPy,DERPx+3,DERPy); //draw the body
...
DERPyH=DERPy-16;
//head left
u8g.setColorIndex(0); //paint the head
...
u8g.drawLine(DERPxH+7,DERPyH+4,DERPxH+5,DERPyH+2); //draw the head
...
if (storm!=1){
   if (SLEEP<3){
  if(MOOD==4){
  u8g.drawPixel(DERPxH+1, DERPyH-4); //happy mouth
...
if(MOOD==3){ 
  u8g.drawPixel(DERPxH+1, DERPyH-4); // smile
  
}
if(MOOD==2){

      u8g.drawPixel(DERPxH+1, DERPyH-3); // neutral
  
}
if(MOOD==1){

      u8g.drawPixel(DERPxH+1, DERPyH-2); // dissatisfied
   
}
 }
}

//draw an animation of eye blinking
switch (glaza) {
 case 1:
u8g.drawPixel(DERPxH-2, DERPyH-10); // frame1
...
u8g.drawPixel(DERPxH-6, DERPyH-9);
    break;
 case 2:
u8g.drawPixel(DERPxH+2, DERPyH-9);  // frame2
u8g.drawPixel(DERPxH+6, DERPyH-9);
u8g.drawPixel(DERPxH-2, DERPyH-9);  
u8g.drawPixel(DERPxH+3, DERPyH-9);  
u8g.drawPixel(DERPxH+4, DERPyH-9);
...
u8g.drawPixel(DERPxH-6, DERPyH-9);
    break;
 case 3:
u8g.drawPixel(DERPxH+6, DERPyH-8); // frame3
u8g.drawPixel(DERPxH-3, DERPyH-8);
u8g.drawPixel(DERPxH+3, DERPyH-8);  
u8g.drawPixel(DERPxH+4, DERPyH-8); 
u8g.drawPixel(DERPxH+2, DERPyH-8);
...
u8g.drawPixel(DERPxH-6, DERPyH-9); 
    break;
 case 4:
u8g.drawPixel(DERPxH+6, DERPyH-7); // frame4
u8g.drawPixel(DERPxH-3, DERPyH-7);
u8g.drawPixel(DERPxH+2, DERPyH-7); 
...
u8g.drawPixel(DERPxH-6, DERPyH-8); 
    break;
 case 5:
u8g.drawPixel(DERPxH+7, DERPyH-8); // frame5
u8g.drawPixel(DERPxH-6, DERPyH-7); 
    break;
 case 6:
u8g.drawPixel(DERPxH+7, DERPyH-8);  // frame6
u8g.drawPixel(DERPxH-6, DERPyH-7); 
    break;
}
...
if (hmmm<4){
if (nya!=1){  
u8g.drawLine(DERPxH+2,DERPyH-6,DERPxH+5,DERPyH-6); //eyes squint
u8g.drawPixel(DERPxH-4, DERPyH-6); 
  if (night==0){
  SLEEP=0;
   static unsigned long timer = millis(); //timer eyes closed-opened
      if (millis() - timer > 30) {      
        if (direct1==1) {
          if (glaza!=6) {
           glaza++;
        ...
        if (direct1==0) {
           ...
           glaza--;     
            ...
            static unsigned long timer = millis(); //blink timer
 ...
 u8g.drawLine(DERPxH-2,DERPyH-7,DERPxH-2,DERPyH-8); //draw open eyes
...
u8g.drawPixel(DERPxH-2, DERPyH-11); // nose?
u8g.drawPixel(DERPxH-2, DERPyH-7); 
  ...
  u8g.setFont(rus7x13);
   ...
   if(position13==0){
    ...
    if(up==LOW){
      ...
      TIMERset++; //2 - internal, 1 - module, 0 - OFF
...
u8g.drawPixel( DERPxH+16, DERPyH-7);  
        u8g.drawStr(DERPxH+33,DERPyH-18, "Yay!");          
     }
...
u8g.drawPixel( DERPxH+19, DERPyH-9);
u8g.drawPixel( DERPxH+16, DERPyH-7);  
  u8g.drawStr(DERPxH+26,DERPyH-18, "Want!");
}
